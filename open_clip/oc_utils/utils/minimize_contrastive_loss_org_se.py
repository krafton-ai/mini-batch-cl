...
parser.add_argument("--d_data", type=int, default=4)
parser.add_argument("--only_linear", default=False, action='store_true')
parser.add_argument("--optimizer_type", choices=["SGD", "Adam"], default="SGD")
parser.add_argument("--tag", type=str, default="hst_se_")
...
print("d_data", args.d_data)
print("only_linear", args.only_linear)
...
exp_tag = f"{time_tag}_{tag}N{N}_d{d}_lr{lr_full}_s{NUM_STEPS}x{num_steps_factor}_all_Bs{batch_size_list}_linear"
output_dir = f"output/{exp_tag}"
...
def get_embeddings(N, d, requires_grad):
    u = torch.randn((N, d), requires_grad=requires_grad, device=device)
    v = torch.randn((N, d), requires_grad=requires_grad, device=device)
    with torch.no_grad():
...
class Net(nn.Module):
    def __init__(self, d, d_data, only_linear=False, mf=256):
        super().__init__()

        self.u_encoder = nn.Sequential(OrderedDict([
            # ('fc1', nn.Linear(d_data, mf * d_data)),
            ('fc1', nn.Linear(d_data, d)),
            ('relu1', nn.LeakyReLU() if not only_linear else nn.Identity()),
            # ('fc2', nn.Linear(mf * d_data, mf * d_data)),
            # ('relu2', nn.LeakyReLU() if not only_linear else nn.Identity()),
            # ('fc3', nn.Linear(mf * d_data, d)),
        ]))
        self.v_encoder = nn.Sequential(OrderedDict([
            # ('fc1', nn.Linear(d_data, mf * d_data)),
            ('fc1', nn.Linear(d_data, d)),
            ('relu1', nn.LeakyReLU() if not only_linear else nn.Identity()),
            # ('fc2', nn.Linear(mf * d_data, mf * d_data)),
    ...
    def forward(self, u, v):
        u_emb = self.u_encoder(u.detach())
        v_emb = self.v_encoder(v.detach())
        return u_emb, v_emb
...
u_data, v_data = get_embeddings(N, d_data, False)
assert u_data.shape[-1] == v_data.shape[-1] == d_data
assert u_data.requires_grad == v_data.requires_grad == False
model = Net(d, d_data, only_linear=only_linear).to(device)
# show model size
model_size = 0
for param in model.parameters():
    model_size += param.data.nelement()
print("Mdel params: %.2f K" % (model_size / 1024))

loss_dict = {}
optimizer = optimizer_fn(model.parameters(), lr=args.lr_full)
for step in tqdm(range(NUM_STEPS*num_steps_factor), desc=f"Full-batch training"):
    optimizer.zero_grad()
    u1, v1 = model(u_data.detach(), v_data.detach())
    # assert u1.shape[-1] == v1.shape[-1] == d
...
for B in batch_size_list:
    ...
    for step in tqdm(range(NUM_STEPS*num_steps_factor), desc=f"[batch_selection:'All NcB' | B:{B}] "):
        optimizer.zero_grad()
        u2, v2 = model(u_data.detach(), v_data.detach())
        ...
        if step %logging_step == 0 or step == NUM_STEPS-1:
            # print("B={} | Step={} | Loss={}".format(B, step, loss))
        ...
        for step in tqdm(range(NUM_STEPS*num_steps_factor), desc=f"[batch_selection:'{batch_selection}' | B:{B}] "):
            if batch_selection == 'osgd':
                ...
                for _ in range(len(batch_idxs)): # different meaning of epochs in here
                    u3, v3 = model(u_data.detach(), v_data.detach())
                    with torch.no_grad():
