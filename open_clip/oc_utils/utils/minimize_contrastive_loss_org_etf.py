...
def etf_full_batch_loss(u, v, off_diagonal):
    labels = torch.eye(u.shape[0], device=device)
    labels[labels == 0.] = off_diagonal
    u = F.normalize(u, p=2.0, dim = 1)
    v = F.normalize(v, p=2.0, dim = 1)
    logits = u @ v.T
    return F.l1_loss(labels, logits)

def etf_clip_batch_loss(u, v, off_diagonal=0.0):
    return etf_full_batch_loss(u, v, off_diagonal=off_diagonal)\
         + etf_full_batch_loss(v, u, off_diagonal=off_diagonal)


def mini_batch_loss(u, v, batch_idxs=None, B=2, off_diagonal=0.0):
    loss = 0
    if batch_idxs == None:
    ...
    for batch_idx in batch_idxs:
        ...
        loss += etf_clip_batch_loss(u_batch, v_batch, off_diagonal)
    return loss/n
...
output_dir = f"output/etf_hst_N{N}_d{d}_lr{lr_full}_s{NUM_STEPS}x{num_steps_factor}_all_Bs{batch_size_list}"
...
# for step in tqdm(range(NUM_STEPS*num_steps_factor)):
#     optimizer.zero_grad()
        ...
        for step in tqdm(range(NUM_STEPS*num_steps_factor), desc=f"[batch_selection:'{batch_selection}' | B:{B}] "):
            if batch_selection == 'osgd':
                ...
                for _ in range(len(batch_idxs)): # different meaning of epochs in here
                    ...
                    loss = etf_clip_batch_loss(u_batch, v_batch, off_diagonal=off_diagonal) #+ clip_batch_loss(u, v)
                    loss.backward()
                    optimizer.step()
                    total_loss += loss
                ...
                optimizer.zero_grad()
                loss = mini_batch_loss(u3, v3, batch_idxs=batch_idxs, off_diagonal=off_diagonal)
                loss.backward()
                optimizer.step()
            ...
            if step %logging_step == 0 or step == NUM_STEPS-1:
                ...
                loss_dict[step], true_loss_dict[step] = loss.item(), etf_clip_batch_loss(u3, v3, off_diagonal=off_diagonal).detach().item()
